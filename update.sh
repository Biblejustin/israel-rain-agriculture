...
# old. Pinned CCKP baseline stays fixed; CRU-CY4.10 refreshes separately.
# WDI and official Kinneret refresh each run. PYTHON may select a clean runtime.
set -u
...
"$PY" "$HERE/monitor_climate.py" > "$HERE/results/climate_latest.txt" 2>&1 || { cat "$HERE/results/climate_latest.txt" >&2; exit 1; }
